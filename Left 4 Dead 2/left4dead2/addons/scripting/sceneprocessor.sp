// ============================================================================
// Scene Processor (with pitch normalization + first-scene and per-client guards)
// Fork of Mr. Zero / cravenge / Dragokas version with integrated pitch fix
// Version: 1.33.3-pitchfix2
// ============================================================================
...
#if !defined DEFAULT_SCENE_PITCH
    #define DEFAULT_SCENE_PITCH 100.0
#endif

#if !defined DEFAULT_SCENE_PREDELAY
    #define DEFAULT_SCENE_PREDELAY 0.0
#endif
...
// Pitch normalization CVARs/state
ConVar sp_enforce_pitch;          bool  g_bEnforcePitch = true;
ConVar sp_pitch_min;              float g_fPitchMin     = 95.0;
ConVar sp_pitch_max;              float g_fPitchMax     = 105.0;
ConVar sp_fix_first_scene;        bool  g_bFixFirst     = true;

bool g_bFirstSceneFixed = false;
bool g_bFirstSceneFixedClient[MAXPLAYERS+1];

// Jailbreak vocalize CVAR handle for safe comparisons
ConVar g_hJailbreakVocalize = null;

static void CachePitchCvars()
{
    if (sp_enforce_pitch != null)   g_bEnforcePitch = sp_enforce_pitch.BoolValue;
    if (sp_pitch_min != null)       g_fPitchMin     = sp_pitch_min.FloatValue;
    if (sp_pitch_max != null)       g_fPitchMax     = sp_pitch_max.FloatValue;
    if (sp_fix_first_scene != null) g_bFixFirst     = sp_fix_first_scene.BoolValue;

    if (g_fPitchMin <= 1.0)  g_fPitchMin = 1.0;
    if (g_fPitchMax < g_fPitchMin) g_fPitchMax = g_fPitchMin;
}

static void NormalizeScenePitchAtSpawn(int entity)
{
    if (!g_bEnforcePitch) return;

    int actor = nSceneData[entity].iActorData;
    bool firstForClient = (actor > 0 && actor <= MaxClients && IsClientInGame(actor) && !g_bFirstSceneFixedClient[actor]);

    float p = nSceneData[entity].fPitchData;
    bool outOfRange = (p <= 0.0) || (p < g_fPitchMin) || (p > g_fPitchMax);

    if ((g_bFixFirst && !g_bFirstSceneFixed) || firstForClient || outOfRange)
    {
        float tgt = DEFAULT_SCENE_PITCH;
        SetEntPropFloat(entity, Prop_Data, "m_fPitch", tgt);
        nSceneData[entity].fPitchData = tgt;

        if (firstForClient) g_bFirstSceneFixedClient[actor] = true;
        if (g_bFixFirst && !g_bFirstSceneFixed) g_bFirstSceneFixed = true;
    }
}

static void NormalizeScenePitchAtStart(int entity)
{
    if (!g_bEnforcePitch) return;
    ...
    float p = GetEntPropFloat(entity, Prop_Data, "m_fPitch");
    bool outOfRange = (p <= 0.0) || (p < g_fPitchMin) || (p > g_fPitchMax);

    if ((g_bFixFirst && !g_bFirstSceneFixed) || firstForClient || outOfRange)
    {
        SetEntPropFloat(entity, Prop_Data, "m_fPitch", DEFAULT_SCENE_PITCH);
        nSceneData[entity].fPitchData = DEFAULT_SCENE_PITCH;
...
public Plugin myinfo = 
{
	...
	author = "Buster \"Mr. Zero\" Nielsen (Fork by cravenge & Dragokas) + pitchfix2",
...
public void OnPluginStart()
{
    ...
    sp_enforce_pitch   = CreateConVar("sceneprocessor_enforce_pitch", "1", "Clamp/normalize scene pitch at spawn/start", FCVAR_SPONLY|FCVAR_NOTIFY);
    sp_pitch_min       = CreateConVar("sceneprocessor_pitch_min",     "95.0", "Min allowed pitch before normalization",  FCVAR_SPONLY|FCVAR_NOTIFY);
    sp_pitch_max       = CreateConVar("sceneprocessor_pitch_max",     "105.0","Max allowed pitch before normalization", FCVAR_SPONLY|FCVAR_NOTIFY);
    sp_fix_first_scene = CreateConVar("sceneprocessor_fix_first_scene","1",  "Force default pitch on first scene of map", FCVAR_SPONLY|FCVAR_NOTIFY);

    sp_enforce_pitch.AddChangeHook(OnSPCVarChanged);
    sp_pitch_min.AddChangeHook(OnSPCVarChanged);
    sp_pitch_max.AddChangeHook(OnSPCVarChanged);
    sp_fix_first_scene.AddChangeHook(OnSPCVarChanged);
    CachePitchCvars();

	if (!bIsL4D1)
	{
		g_hJailbreakVocalize = CreateConVar("sceneprocessor_jailbreak_vocalize", "1", "Enable/Disable Jailbreak Vocalizations", FCVAR_SPONLY|FCVAR_NOTIFY);
		g_hJailbreakVocalize.AddChangeHook(OnSPCVarChanged);
		bJailbreakVocalize = g_hJailbreakVocalize.BoolValue;
	}

    for (int i = 1; i <= MaxClients; i++)
        g_bFirstSceneFixedClient[i] = false;
...
{
	if (!bIsL4D1 && g_hJailbreakVocalize != null && cvar == g_hJailbreakVocalize)
	{
...
{
    g_bFirstSceneFixedClient[client] = false;
...
public void OnSpawnPost(int entity)
{
    ...
    // Pitch normalization at spawn
    NormalizeScenePitchAtSpawn(entity);
...
public void OnSceneStart_EntOutput(const char[] output, int caller, int activator, float delay)
{
	...
	{
        // Re-assert pitch at OnStart (covers engine mutation between spawn and start)
        NormalizeScenePitchAtStart(caller);
...
void Scene_Perform(int client, const char[] sVocalizeParam, const char[] sFileParam = "", float fScenePreDelay = DEFAULT_SCENE_PREDELAY, float fScenePitch = DEFAULT_SCENE_PITCH, int iSceneInitiator = SCENE_INITIATOR_PLUGIN, bool bVocalizeNow = false)
{
	if (sFileParam[0] && FileExists(sFileParam, true))
	{
        ...
        NormalizeScenePitchAtSpawn(iScene);
