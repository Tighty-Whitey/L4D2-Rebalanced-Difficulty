...
#define PIPEBOMB_SOUND "weapons/hegrenade/beep.wav"
#define MOLOTOV_SOUND "weapons/molotov/fire_ignite_2.wav"
...
int g_iAlreadyThrownEntityRef[2049] = {INVALID_ENT_REFERENCE, ...};
...
public void OnMapEnd()
{
	...
	{
		g_iAlreadyThrownEntityRef[i] = INVALID_ENT_REFERENCE;
	}
}
...
public Action HandheldSoundHook(int iClients[64], int &iNumClients, char sSampleFile[PLATFORM_MAX_PATH], int &iEntity, int &iChannel, float &fVolume, int &fLevel, int &iPitch, int &iFlags)
{
	...
	if(IsValidEntRef(g_iAlreadyThrownEntityRef[iEntity]))
		return Plugin_Continue;
	
	g_iAlreadyThrownEntityRef[iEntity] = EntIndexToEntRef(iEntity);
